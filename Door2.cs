        ...
        while (true)
        {
            if (moveDoor)
            {
                ...
                if (doorCollision == DoorCollision.FRONT)
                {
                    ...
                    if (Mathf.Abs(angle - lastAngle) > creakAng && (this.gameObject.transform.localEulerAngles.z > 0
                        && this.gameObject.transform.localEulerAngles.z < backOpenPosLimit))
                    {
                ...
                else if (doorCollision == DoorCollision.BACK)
                {
                    ...
                    Stamina.instance.normalWalk = 1f;
                    angle += Input.GetAxis("Mouse X");
                    if (Mathf.Abs(angle - lastAngle) > creakAng && (this.gameObject.transform.localEulerAngles.z < backOpenPosLimit
                        && this.gameObject.transform.localEulerAngles.z > 0))
                    {
                        lastAngle = angle; // update lastAngle
                        float deltaT = Time.time - lastCreak; // calc time from last creak
                        lastCreak = Time.time;
